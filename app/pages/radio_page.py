...
class RadioPage(CrewPage):
    ...
    def request_safe_close(self) -> bool:
        ...
        answer = QMessageBox.question(
            self,
            "任务正在运行",
            "无线电平均分配任务仍在运行。是否安全取消并在当前文件完成后关闭？",
    ...
    def _change_copy_mode(self, average: bool) -> None:
        if self.is_busy or self._manual_batch_state in (
            ManualBatchState.RUNNING,
            ManualBatchState.RESULT_RETAINED,
        ):
            self.copy_mode_switch.set_average_checked(
                self._manual_copy_mode is ManualCopyMode.AVERAGE, emit=False
            )
            return
    ...
    def _logical_selections(self) -> dict[tuple[str, str], bool]:
        ...
        for plan in self._group_plans:
            group_id = plan.group.group_key
            for assignment in plan.assignments:
                logical[(group_id, assignment.target_name)] = self._selected_targets.get(
                    assignment.selection_key, True
                )
        return logical
    ...
    def _add_paths(self, paths: Iterable[str]) -> None:
        ...
        for raw_path in paths:
            ...
            if source.normalized_path in known_paths:
                ...
                continue
            if (
                self._manual_copy_mode is ManualCopyMode.AVERAGE
                and source.is_recognized
                and source.group_key is not None
            ):
        ...
        messages: list[str] = []
        if accepted:
            messages.append(f"已导入 {len(accepted)} 个文件")
        ...
        if rejected:
            messages.append(f"已拒绝 {sum(len(items) for items in rejected.values())} 个超过平均分配上限的文件")
        if messages:
    ...
    def _refresh_sources_and_groups(self) -> None:
        if (
            self._manual_copy_mode is ManualCopyMode.SEQUENTIAL
            or self._radio_average_builder is None
        ):
            super()._refresh_sources_and_groups()
            return
        self.source_list.set_files(self._imported_files)
        self._group_plans = self._radio_average_builder.build_group_plans(
            self._imported_files, self._radio_import_order
        )
        valid_keys = {
    ...
    def _render_confirmation_groups(self) -> None:
        submitted = self._manual_batch_state in (
            ManualBatchState.RUNNING,
            ManualBatchState.RESULT_RETAINED,
        )
    ...
    def _start_copy(self) -> None:
        ...
        if self.is_busy or self._manual_batch_state is not ManualBatchState.EDITING:
            return
        plan = self._require_average_builder().build_staged_copy_plan(
            self._group_plans, self._selected_targets
        )
        if plan.total == 0:
            self._manual_scroll_guard.preserve()
    ...
    def _choose_average_conflict_policy(
        self,
        ...
        dialog.setWindowTitle("发现外部目标文件冲突")
        dialog.setText(f"本批任务中有 {len(existing_paths)} 个外部目标文件已存在。")
        dialog.setInformativeText(
            "本次导入源文件所对应的目标会在安全暂存后按平均分配计划重写；"
            "以下选项仅应用于其他已存在文件。"
        )
        dialog.setDetailedText("\n".join(str(path) for path in existing_paths))
        skip_button = dialog.addButton("跳过外部冲突文件", QMessageBox.ButtonRole.AcceptRole)
        overwrite_button = dialog.addButton("覆盖外部冲突文件", QMessageBox.ButtonRole.DestructiveRole)
    ...
    def _show_average_results(self, result: CopyBatchResult) -> None:
        ...
        self.results_summary.setText(
            f"新建 {result.created}，覆盖 {result.overwritten}，跳过 {result.skipped}，"
            f"失败 {result.failed}，取消 {result.cancelled}。"
        )
        failures = [item for item in result.results if item.status is CopyResultStatus.FAILED]
        if failures:
            dialog = QMessageBox(self)
            dialog.setIcon(QMessageBox.Icon.Warning)
            dialog.setWindowTitle("部分无线电文件复制失败")
            dialog.setText(f"本批任务有 {len(failures)} 个文件失败。")
            dialog.setDetailedText(
    ...
    def _resolve_average_conflicts(self, paths: tuple[Path, ...]) -> None:
        ...
        dialog.setIcon(QMessageBox.Icon.Warning)
        dialog.setWindowTitle("发现外部目标文件冲突")
        dialog.setText(f"本批任务中有 {len(paths)} 个外部目标文件已存在。")
        dialog.setInformativeText("请选择对本批所有外部冲突文件统一应用的处理方式。")
    ...
    def _sync_interaction_state(self, *_args: object) -> None:
        ...
        if not hasattr(self, "copy_mode_switch"):
            return
        locked = self._manual_batch_state in (
            ManualBatchState.RUNNING,
            ManualBatchState.RESULT_RETAINED,
        )
    ...
    def _show_fallback_warning_once(self) -> None:
        if self._manual_copy_mode is not ManualCopyMode.AVERAGE:
            return
        fallback = tuple(
            f"{plan.group.base_name}\n  {plan.directory}"
            for plan in self._group_plans
            if isinstance(plan, RadioManualGroupPlan) and plan.fallback_reason
        )
        ...
        dialog.setWindowTitle("部分组别已回退顺序复制")
        dialog.setText("当前组别无法使用平均分配模式，已自动顺序分配")
        dialog.setDetailedText("\n\n".join(fallback))
        dialog.exec()
    ...
    def _show_limit_violations(self, violations: tuple[object, ...], *, switching: bool) -> None:
        dialog = QMessageBox(self)
        dialog.setIcon(QMessageBox.Icon.Warning)
        dialog.setWindowTitle("无法切换到平均分配" if switching else "超过平均分配导入上限")
        dialog.setText(
            "当前文件数超过平均分配上限，已保持顺序复制。"
            if switching
            else "部分文件超过平均分配上限，未加入列表。"
        )
        dialog.setDetailedText(
            "\n\n".join(
                f"项目组：{item.group_name}\n源目录：{item.directory}\n"
                f"成员数：{item.member_count}\n最多允许：{item.maximum}\n实际导入：{item.actual}"
    ...
    def _show_rejected_imports(self, rejected: dict[tuple[str, str], list[tuple[object, Path]]]) -> None:
        ...
        for (group_key, _scope), items in rejected.items():
            ...
            if group is None or maximum is None:
                continue
            accepted_count = sum(
                source.group_key == group_key and self._scope_key(source.path.parent) == _scope
                for source in self._imported_files
            )
            details.append(
                f"项目组：{group.base_name}\n成员数：{len(group.names)}\n最多允许：{maximum}\n"
                f"实际尝试：{accepted_count + len(items)}\n拒绝文件：\n"
                + "\n".join(f"  {path}" for _source, path in items)
            )
        dialog = QMessageBox(self)
        dialog.setIcon(QMessageBox.Icon.Warning)
        dialog.setWindowTitle("超过平均分配导入上限")
        dialog.setText("已保留此前成功导入的文件，超出上限的文件未加入列表。")
        dialog.setDetailedText("\n\n".join(details))
        dialog.exec()

    def _require_average_builder(self) -> RadioAveragePlanBuilder:
        if self._radio_average_builder is None:
            raise RuntimeError("无线电平均分配构建器尚未初始化。")
