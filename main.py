...
from app.branding import APP_NAME, ICON_RESOURCE, WINDOW_TITLE
...
def main() -> int:
    ...
    app.setOrganizationName("Beiku")
    app.setQuitOnLastWindowClosed(True)
    logger = configure_logging(); install_exception_hooks(logger)
    logger.info("Application startup")
    apply_theme(app)
    icon = QIcon(ICON_RESOURCE)
    if icon.isNull(): logger.warning("Application icon resource is unavailable")
    else: app.setWindowIcon(icon)
    ...
    try:
        window = MainWindow(); window.show(); window.activateWindow(); logger.info("Main window displayed")
        exit_code = app.exec()
